...
<script>
  ...
  export default {
    ...
    methods: {
      resetLint() {
        if (!this.$refs['codemirror'].codemirror.getValue()) {
          this.$nextTick(() => {
      ...
      resetFoldGutter() {
        this.$refs['codemirror'].codemirror.setOption("foldGutter", false);
        this.$nextTick(() => {
          this.$refs['codemirror'].codemirror.setOption("foldGutter", true);
        })
      },
      ...
      setStyle(style) {
        try {
          this.$nextTick(() => {
            let cm = this.$refs['codemirror'].$el.querySelector(".CodeMirror");
            if (cm) {
      ...
      setValue(value) {
        try {
          ...
          if (this.cmOptions.mode == "application/json") {
            this.$nextTick(_ => this.editorValue = this.formatStrInJson(value))
          } else {
            this.$nextTick(_ => this.editorValue = value)
          }
        } catch (e) {
      ...
      // 按下鼠标时事件处理函数
      onMouseDown(event) {
        this.$refs['codemirror'].codemirror.closeHint()
      },
